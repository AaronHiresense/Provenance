...
export function TopBar({ theme, onToggleTheme, result }: Props) {
  const engine = result ? (result.llm_provider === "mock" ? "Offline" : `${result.llm_provider}${result.llm_model ? ` · ${result.llm_model}` : ""}`) : "Ready";
  return (
